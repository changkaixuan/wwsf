﻿﻿define([
	'underScore', 
	'text!application/wws_edit_table.html',
	'css!js/bootstrap/css/bootstrap-table.css'],
    function (_, template) {
		var app = function (pA,pInput,pSpan,pFirstModalId,pJobId) {
	        this.o_template = $(template);
	        this.o_a = pA;
	        this.o_input = pInput;
	        this.o_span = pSpan;
	        this.o_firstModalId = pFirstModalId;
	        this.o_jobId = pJobId;
	    }
        _.extend(app.prototype, {
            ...
            render: function () {
            	var self = this;
            	if(self.o_firstModalId != null && self.o_firstModalId != ""){
            		this.o_template.insertAfter($("#"+self.o_firstModalId)); //当前modal追加到第1层modal/<a>下面（x-editable不支持嵌套，同级没问题）
            	}else{
            		this.o_template.insertAfter(this.o_a); //将modal追加到<a>后面
            	}
            	var tModal = $("#wwsEditTableModal");
            	var dataJson = [];//需要显示的表格数据
            	var aInputValue = self.o_input.value;
            	if(aInputValue != null && aInputValue != ""){//输入框为空就加载一条空数据
            		var parametersJson = JSON.parse(aInputValue);
            		$.ajax({
                		async:false,
                		url: 'template/getJobParameter?jobId='+self.o_jobId,
            			type : 'GET',
            	        ...
            	        success:function (data) {
            	        	if(data.success){
            					...
            					for(var key in aInputValueJosn){
            						aKeyArr.push(key);
        							aValueArr.push(aInputValueJosn[key]);
        							aJobParameterArr.push({key:key,value:aInputValueJosn[key],isMust:0,tempParamType:0});
            					}
            					...
            					if(jobParameters != undefined){//JOB有参数
            						$.each(jobParameters,function(i){
            							...
            							if(index == -1){//JOB参数不在"链接的值/parameters"中，所有属性值以JOB参数为准
            								dataJson.push({key:jobParameters[i].name,value:jobParameters[i].value,isMust:jobParameters[i].required,tempParamType:1});
            							}else{//JOB参数在"链接的值/parameters"中，值以"链接的值/parameters/value"为准
            								dataJson.push({key:jobParameters[i].name,value:aValueArr[index],isMust:jobParameters[i].required,tempParamType:1});
            							}
            							jobKeyArr.push(jobParameters[i].name);
            						});
            						//将新增的参数追加进来
            						for(var i=0;i<aJobParameterArr.length;i++){
            							var index =  $.inArray(aJobParameterArr[i].key, jobKeyArr);
            							if(index == -1){//链接的值/parameters/key,不在JOB参数中，认为是新增的参数
            								dataJson.push(aJobParameterArr[i]);
            							}
            						}
            					}else{//JOB没有参数，以链接的值/parameters为准
            						dataJson = aJobParameterArr;
            					}
            					self.initEditTable("#tab_wwsEditTable",dataJson,"/tabWwsEditTable","/updateTabWwsEditTable","属性","值","是否必填");
            	        	} else {
            	...
            	self.initEditTable("#tab_wwsEditTable",dataJson,"/tabWwsEditTable","/updateTabWwsEditTable","属性","值","是否必填");
            	//保存事件
            	tModal.find("button[name=save]").unbind('click').bind('click', function(event){
            		self.saveEditTable();
            	});
            	//关闭事件
            	tModal.find("button[name=close]").unbind('click').bind('click', function(event){
            ...
            initEditTable: function(useId,dataArr,useUrl,useChildUrl,columnKey,columnValue,isMust){
               	// 填充数据
        	...
        	saveEditTable: function(){
        		var tModal = $('#wwsEditTableModal');
        		var parametersJson = {};
        		var rows = $('#tab_wwsEditTable').bootstrapTable("getData");
        		for(var i=0; i<rows.length; i++){
        			if(rows[i].isMust == 1){//必填参数
        				...
        				parametersJson[rows[i].key] = rows[i].value;
        			}else{
        		...
        		this.o_input.value = JSON.stringify(parametersJson);
        		if(this.o_input.value == "{}"){
        			this.o_input.value = "";
        		}
