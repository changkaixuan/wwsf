...
var initApp = (function(app) {
	...
	app.dateFill = function(){
		var myAppDateHook = $('.js-get-date');
		if ( myAppDateHook.length ) {
...
/*****************************************************************************************
 * for cron expression end
 ****************************************/

function genMenuString(sysAuthArr,authId){
	var menuArr = [];
	for(var a=0;a<sysAuthArr.length;a++){
		if(null != sysAuthArr[a].pid && sysAuthArr[a].pid == authId){
			var tMenu = "<li>";
			var title = sysAuthArr[a].name;
			var menuChildArr = genMenuString(sysAuthArr, sysAuthArr[a].id);
			/*
			 	（1）有子节点
				 <a href="#" title="SysManager" data-filter-tags="系统管理">
                <i class="fal fa-user"></i>
                <span class="nav-link-text" data-i18n="nav.wwssys.mgr">系统管理</span>
             </a>
                                                    （2）无子节点
             <a href="#" title="用户管理" data-module="wws_sys_user" data-target="#js-page-content" data-filter-tags="navLink 系统管理  用户管理">
               <span class="nav-link-text" data-i18n="nav.wws_sys_user">用户管理</span>
             </a>
            */
			if(menuChildArr.length > 0){//有子节点
				tMenu += "<a href='#' title='"+title+"' data-filter-tags='"+sysAuthArr[a].dataFilterTags+"'>";
				tMenu += "<i class='"+sysAuthArr[a].iconClass+"'></i>";
				tMenu += "<span class='nav-link-text' data-i18n='"+sysAuthArr[a].id+"'>"+title+"</span>";
    			tMenu += "</a>";
    			tMenu += "<ul>";
    			for(var b=0;b<menuChildArr.length;b++){
    				tMenu += menuChildArr[b];
    			}
    			tMenu += "</ul>";
			}else{
				tMenu += "<a href='#' title='"+title+"' data-filter-tags='"+sysAuthArr[a].dataFilterTags+"' data-module='"+sysAuthArr[a].dataModule+"' data-target='#js-page-content' >";
				if(sysAuthArr[a].iconClass == null || sysAuthArr[a].iconClass == ""){
					tMenu += "<i class='fal fa-long-arrow-alt-right'></i>";
				}else{
					tMenu += "<i class='"+sysAuthArr[a].iconClass+"'></i>";
				}
				tMenu += "<span class='nav-link-text' data-i18n='"+sysAuthArr[a].id+"'>"+title+"</span>";
			...
			tMenu += "</li>";
			if(sysAuthArr[a].requiresPermissions == null || sysAuthArr[a].requiresPermissions == "" || sysAuthArr[a].requiresPermissions.indexOf("menu:") == 0){
				menuArr.push(tMenu);
			}
		}
	}
...
function loadMessage(){
	$.ajax({
        ...
        success:function (data) {
        	var indexMessageObj = $("#index_message");
            if (data.success){
            	if(data.data.newMessageCount == 0){
            		indexMessageObj.find("span[name=newMessageCount]").removeAttr("class");
            		indexMessageObj.find("span[name=newMessageCount]")[0].innerHTML = "";
            		indexMessageObj.find("span[name=newMessageDesc]")[0].innerHTML = "没有新的消息";
            		$("#newMessageTitle").removeAttr("title");
            	}else{
            		indexMessageObj.find("span[name=newMessageCount]").attr("class","badge badge-icon");
            		indexMessageObj.find("span[name=newMessageCount]")[0].innerHTML = data.data.newMessageCount;
            		indexMessageObj.find("span[name=newMessageDesc]")[0].innerHTML = data.data.newMessageCount + " 条新的消息";
            		$("#newMessageTitle").attr("title","您收到 "+data.data.newMessageCount+" 条新的消息");
            	}
            	var indexMessageListObj = $("#index_message_list"); //index.html/ul
            	indexMessageListObj[0].innerHTML = "";
            	if(data.data.messageList != undefined){
            		var messageArr = data.data.messageList;
            		var mLi = "";
            		for(var q=0;q<messageArr.length;q++){
            			mLi += "<li>";
            			mLi +=   "<a href='#' id='index_message_list_a_id_"+messageArr[q].id+"' class='d-flex align-items-center'>";
            			mLi +=     "<span class='d-flex flex-column flex-1 ml-1'>";
            			var tInfoDesc = " "+messageArr[q].createTime+" ";
            			var tMInfo = messageArr[q].mInfo;
            			if(tMInfo.length < 60){
            				tInfoDesc += tMInfo;
            			}else{
            				tInfoDesc += tMInfo.substr(0,60) + "...";
            			}
            			mLi +=       "<span class='msg-a fs-sm'>";
            			if(messageArr[q].mRead == 0){
          				    mLi +=     "<span class='badge badge-info'>未读</span>"+tInfoDesc;	
          			    }else{
          				    mLi +=     "<span class='badge badge-secondary'>已读</span>"+tInfoDesc;	
          			    }
            			mLi +=       "</span>";
            			var tTimeDesc = "";
            			if(messageArr[q].mTimeSec < 60){
            				tTimeDesc += Math.floor(messageArr[q].mTimeSec) + " 秒前";
            			}else if(messageArr[q].mTimeSec >= 60 && messageArr[q].mTimeSec < 60*60){
            				tTimeDesc += Math.floor(messageArr[q].mTimeSec/60) + " 分钟前";
            			}else if(messageArr[q].mTimeSec >= 60*60 && messageArr[q].mTimeSec < 60*60*24){
            				tTimeDesc += Math.floor(messageArr[q].mTimeSec/60/60) + " 小时前";
            			}else if(messageArr[q].mTimeSec >= 60*60*24){
            				tTimeDesc += Math.floor(messageArr[q].mTimeSec/60/60/24) + " 天前";
            			}
            			mLi +=       "<span class='fs-nano text-muted mt-1'>"+tTimeDesc+"</span>";
            			mLi +=     "</span>";	   
            			mLi +=   "</a>";        
            			mLi += "</li>";          
            		}
            		indexMessageListObj.append(mLi);
            	}
            }else{
...
function loadMessageClick(){
	//绑定消息点击事件
	$.each($("#index_message_list a"),function(index,aObj){
		if(aObj.id != null && aObj.id.indexOf("index_message_list_a_id_") == 0){
			var messageId = aObj.id.replace("index_message_list_a_id_","");
			$(aObj).unbind('click').bind('click', function(event){
			    var tMRead = $(this).find("span[class='badge badge-info']"); //未读
			    if(tMRead.length == 1){
			    	$.ajax({
            			async : false,
...
function isNumberWwsf(value){//大于等于0
	if(null != value && value != ""){
		if(value == 0){
			return true;
		}
...
function validateStringLength(inputValue,maxLength){
	if(inputValue != null && inputValue != ""){
		var strlen = inputValue.replace(/[^\x00-\xff]/g, "**").length;
		if(strlen > maxLength){
			return false;
		}
	}
