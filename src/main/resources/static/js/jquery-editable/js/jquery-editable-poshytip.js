        ...
        **/        
        ...
        showLoading: function() {
            ...
            if(this.$form) {
                ...
                if(w) {
                	//additional code by lyx 20190731
...
(function ($) {
    ...
    Popup.prototype = {
        ...
        **/
        ...
        wwsfInput: function(){
        	var wwsfInputStartName = "wwsfInput";
        	var wwsfInputArr = this.tip().find("input[name^=wwsfInput]");
        	if(wwsfInputArr.length > 0){//wwsf自定义输入框
        		var wwsfInputType = wwsfInputArr[0].name.replace(wwsfInputStartName,"");
        		if(wwsfInputType == "Date"){
        			wwsfInputArr.daterangepicker({
    	    			singleDatePicker : true,
    	    			locale : {
    	    				format : 'YYYYMMDD'
    	    			}
    	    		});
        		}else if(wwsfInputType == "Checkbox"){
        			//初始化
        			if(wwsfInputArr[0].value == "true"){
        				wwsfInputArr[0].value = "true";
            			wwsfInputArr[0].checked = true;
        			}else{
        				wwsfInputArr[0].value = "false";
            			wwsfInputArr[0].checked = false;
        			}
        			$(wwsfInputArr).unbind('click').bind('click', function(event){
        				if(this.checked){
        					this.value = "true";
        				}else{
        					this.value = "false";
        		...
        		}else if(wwsfInputType == "DateTime"){
        			wwsfInputArr.daterangepicker({
    	    			singleDatePicker : true,
    	    			timePicker: true,// 显示 时 分
    	    			timePickerSeconds: true,// 显示秒
    	    			timePickerIncrement: 1,// 1刻度 递增
    	    			timePicker24Hour: true,// 24小时制
    	    			locale : {
    	    				format : 'YYYYMMDD HH:mm:ss'
    	    			}
    	    		});
        		}else if(wwsfInputType == "Job"){
        			var pInput = wwsfInputArr[0];
        			var pA = this.$element;
        			var pSpan = this.tip();
        			var pProduct = pSpan.find("input[name=product]").val();
        			require([ "application/wws_job_parameter" ], function(ModuleClass) {
        				var oModule = new ModuleClass(pA,pInput,pSpan,pProduct);
        				oModule.load();
        				oModule.render();
        			});
        		}else if(wwsfInputType == "Jobs"){
        			var pInput = wwsfInputArr[0];
        			var pA = this.$element;
        			var pSpan = this.tip();
        			var pProduct = pSpan.find("input[name=product]").val();
        			require([ "application/wws_jobs_parameter" ], function(ModuleClass) {
        				var oModule = new ModuleClass(pA,pInput,pSpan,pProduct);
        				oModule.load();
        				oModule.render();
        			});
        		}else if(wwsfInputType == "EditTable"){
        			var pInput = wwsfInputArr[0];
        			var pA = this.$element;
        			var pSpan = this.tip();
        			var pFirstModalId = pSpan.find("input[name=firstModalId]").val();
        			var pJobId = pSpan.find("input[name=jobId]").val();
        			require([ "application/wws_edit_table" ], function(ModuleClass) {
        				var oModule = new ModuleClass(pA,pInput,pSpan,pFirstModalId,pJobId);
        				oModule.load();
        				oModule.render();
        			});
        		}
        ...
        renderList: function() {
            ...
            if(this.sourceData.length==1) {
            	for(var i=0; i<this.sourceData.length; i++) {
                    $label = $('<label class="mt-1 float-left">').append($('<input>', {
                                               type: 'checkbox',
